...
\]
For a prime $p$: $\phi(p) = p - 1$. For a prime power: $\phi(p^k) = p^{k-1}(p-1)$.
...
\textbf{Proof Sketch.} Consider the set $S = \{a, 2a, 3a, \ldots, (p-1)a\}$ reduced modulo $p$. Since $\gcd(a, p) = 1$, each element of $S$ is distinct and non-zero modulo $p$. Thus $S \equiv \{1, 2, \ldots, p-1\} \pmod{p}$. Multiplying all elements:
\[
...
\textbf{Proof Sketch.}
$(\Rightarrow)$ Suppose $n$ is Carmichael. If $p^2 \mid n$ for some prime $p$, then choosing $a$ as a primitive root modulo $p^2$ leads to a contradiction with $a^{n-1} \equiv 1 \pmod{p^2}$. Thus $n$ is squarefree. For each prime $p \mid n$, Fermat's Little Theorem gives $a^{p-1} \equiv 1 \pmod{p}$ for any $a$ coprime to $p$. Combined with $a^{n-1} \equiv 1 \pmod{n}$, this forces $(p-1) \mid (n-1)$.

$(\Leftarrow)$ Suppose $n$ is squarefree and $(p-1) \mid (n-1)$ for all primes $p \mid n$. For any $a$ with $\gcd(a, n) = 1$, by Fermat's theorem $a^{p-1} \equiv 1 \pmod{p}$. Since $(p-1) \mid (n-1)$, we have $a^{n-1} \equiv 1 \pmod{p}$. This holds for all prime factors of $n$, so by the Chinese Remainder Theorem, $a^{n-1} \equiv 1 \pmod{n}$. \hfill $\square$
...
\textbf{Probability of Detection.} For a Carmichael number $n = p_1 p_2 \cdots p_k$, the only bases that reveal compositeness are those sharing a factor with $n$. The probability of randomly selecting such a ``lucky'' witness is:
\[
\Pr[\gcd(a, n) > 1] = 1 - \frac{\phi(n)}{n} = 1 - \prod_{i=1}^{k}\left(1 - \frac{1}{p_i}\right)
\]
For $n = 561 = 3 \times 11 \times 17$, this probability is approximately $40\%$, meaning roughly $60\%$ of bases give no information about compositeness.
...
\subsection{Derivation of the Miller-Rabin Test}

\subsubsection{From Fermat to Miller-Rabin}
...
\begin{algorithmic}[1]
\Function{IsPrime}{$n, k$} \Comment{$n$ is number to test, $k$ is rounds}
    \If{$n = 2$ or $n = 3$} \Return{True} \EndIf
    \If{$n$ is even} \Return{False} \EndIf
    
    \State Write $n-1$ as $2^r \cdot d$
    
    \For{$i \gets 1$ to $k$}
        \State $a \gets \text{random integer in } [2, n-2]$
        \State $x \gets a^d \pmod n$
        \If{$x = 1$ or $x = n-1$}
            \State \textbf{continue}
        \EndIf
        \For{$j \gets 1$ to $r-1$}
            \State $x \gets x^2 \pmod n$
            \If{$x = n-1$}
                \State \textbf{break loop} \Comment{$n$ passes this round}
            \EndIf
        \EndFor
        \If{loop finished without break} \Return{Composite} \EndIf
    \EndFor
...
\textbf{Proof Sketch.} The proof uses group theory. The strong liars form a proper subgroup of $(\mathbb{Z}/n\mathbb{Z})^*$. By analyzing the structure of this subgroup (particularly using the Chinese Remainder Theorem when $n$ has multiple prime factors), one can show its index is at least 4. Thus strong liars comprise at most $1/4$ of the group. \hfill $\square$
...
\subsubsection{The \texorpdfstring{$O((\ln n)^2)$}{O((ln n)²)} Bound Under GRH}
A fundamental question is: how many bases must we test to guarantee correctness for \textit{any} $n$?

\begin{theorem}[Miller, 1976; conditional on GRH]
Assuming the Generalized Riemann Hypothesis (GRH), for any composite $n$, there exists a strong witness $a$ with:
\[
a \leq 2(\ln n)^2
\]
\end{theorem}

\textbf{Proof Intuition.} The argument relies on the distribution of quadratic residues and character sums over $(\mathbb{Z}/n\mathbb{Z})^*$. Under GRH:
\begin{enumerate}
    \item The smallest quadratic non-residue modulo a prime $p$ is $O((\ln p)^2)$.
    \item For composite $n$, similar bounds apply to finding elements that expose the structure of $(\mathbb{Z}/n\mathbb{Z})^*$.
    \item A strong witness must exist among the first $O((\ln n)^2)$ positive integers.
\end{enumerate}

\textbf{Practical Implication.} Under GRH, testing all bases $a \in \{2, 3, \ldots, \lfloor 2(\ln n)^2 \rfloor\}$ gives a \textit{deterministic} polynomial-time primality test. For a 1024-bit number, this means testing roughly $2 \times (1024 \times \ln 2)^2 \approx 10^6$ bases---still feasible, though slower than probabilistic testing.

\subsubsection{Complexity Analysis}
\textbf{Per-round complexity:} Each Miller-Rabin round requires computing $a^d \bmod n$ and up to $r$ squarings:
\begin{itemize}
    \item Modular exponentiation: $O(\log d) = O(\log n)$ multiplications
    \item Each multiplication of $b$-bit numbers: $O(b^2)$ using schoolbook, or $O(b \log b)$ using FFT-based methods
    \item Total per round: $O((\log n)^3)$ bit operations (schoolbook) or $O((\log n)^2 \log\log n)$ (FFT)
\end{itemize}

\textbf{Probabilistic Miller-Rabin:} With $k$ rounds, complexity is $O(k \cdot (\log n)^3)$.

\textbf{Deterministic Miller-Rabin (under GRH):} Testing $O((\ln n)^2)$ bases gives complexity $O((\log n)^5)$.

\textbf{Comparison with AKS:} The AKS algorithm (2002) provides unconditional deterministic polynomial-time primality testing with complexity $\tilde{O}((\log n)^{6})$, but Miller-Rabin remains faster in practice.
...
% -------------------------------------------------------------------
% METHODS AND ALGORITHMIC ANALYSIS
% -------------------------------------------------------------------
\section{Methods, Proofs, and Algorithmic Analysis}

This section provides a consolidated view of all primality testing algorithms discussed, with pseudocode descriptions and rigorous complexity analysis.
...
\textbf{Correctness:} Always correct when returning ``Composite.'' May incorrectly return ``Probably Prime'' for Carmichael numbers.

\subsubsection{Miller-Rabin Primality Test}

The standard probabilistic test for cryptographic applications.

\begin{algorithm}
\caption{Miller-Rabin Primality Test (Detailed)}
\begin{algorithmic}[1]
...
\textbf{Correctness:} Never returns ``Composite'' for a prime. For any composite $n$, at least $3/4$ of bases are strong witnesses.
...
The graph illustrates why Trial Division becomes infeasible beyond $\sim$60 bits, while Miller-Rabin remains practical for thousands of bits.
